...
//    }
    ...
    public function union_of_automatic_vouchers_and_mass_vouchers_view_generator()
    {
        /*$this->db->select('voucher_journal.id as voucher_id, voucher_journal.trip_product_detail_id as trip_detail_id');
        $this->db->where('voucher_journal.auto_generated','1');
        $this->db->where('voucher_journal.active',1);
        $result1 = $this->db->get('voucher_journa')->result();*/

        /*$this->db->select('trip_detail_voucher_relation.voucher_id, trip_detail_voucher_relation.trip_detail_id');
        $this->db->where('trip_detail_voucher_relation.trip_detail_id !=', 0);
        $result2 = $this->db->get('trip_detail_voucher_relations')->result();
        var_dump($result2); die();*/
    }

    public function manage_account_white_oil_view_generator()
    {
        /*$select =
            '
                id, trip_id, source, destination, product, product_quantity,
                 qty_at_destination, qty_after_decanding, price_unit, freight_unit,
                  company_freight_unit, stn_number,
                   ( CASE
                        WHEN trips_details.shortage_voucher_decnd = "0" THEN shortage_voucher_dest
                        ELSE shortage_voucher_decnd
                     END
                   ) AS shortage_voucher,
                   bill_id,
            ';
        ...
        $result = $this->db->get('trips_detail')->result();
        var_dump($result);
        ...
        $shortage_amount = "
        ( CASE
            WHEN trips_details.shortage_voucher = 0 THEN '0'
            ELSE (shortage_voucher.shortage_quantity * shortage_voucher.price_unit)
          END
        )
        ";
        $total_freight_cmp = "(trips_details.product_quantity * trips_details.company_freight_unit)";
        $wht_amount = "(trips.company_commission_2 * ".$total_freight_cmp." / 100)";
        $company_commission_amount = "(trips.company_commission_1 * ".$total_freight_cmp." / 100)";;
        $contractor_freight = "(100 - trips.company_commission_2)";
        $contractor_freight_amount = "(".$total_freight_cmp." - ".$wht_amount.")";
        $contractor_freight_without_shortage = "(".$contractor_freight_amount." - ".$shortage_amount.")";
        $contractor_net_freight = "(".$contractor_freight_amount." - ".$company_commission_amount.")";
        $contractor_commission = "(trips.contractor_commission - trips.company_commission_2 - trips.company_commission_1)";
        $contractor_commission_amount = "(".$contractor_commission." * ".$total_freight_cmp." / 100)";
        $total_freight_cst = "(trips_details.product_quantity * trips_details.freight_unit)";
        $customer_freight = "(100 - trips.contractor_commission)";
        $customer_freight_amount = "(".$customer_freight." * ".$total_freight_cst." / 100)";
        $customer_freight_without_shortage = "(".$customer_freight_amount." - ".$shortage_amount.")";


        $select = '
            trips.id as trip_id, trips_details.id as trip_detail_id, trips.type as trip_type_id, trip_types.trip_type, trip_types.trip_sub_type,
             trips.entryDate as trip_date, trips_details.product as product_id, trips.tanker_id as tanker_id,
            tankers.truck_number as tanker_number, products.productName, trips_details.product_quantity,
            '.$shortage_amount.' as shortage_amount, source_cities.cityName as source, trips_details.source as source_id,
            destination_cities.cityName as destination, trips_details.destination as destination_id,
             trips_details.stn_number, companies.name as company, trips.company_id, trips.contractor_id, trips.customer_id,
            trips_details.company_freight_unit, '.$total_freight_cmp.' as total_freight_cmp, trips.company_commission_2 as wht,
            '.$wht_amount.' as wht_amount, trips.company_commission_1 as company_commission,
            '.$company_commission_amount.' as company_commission_amount, carriage_contractors.name as contractor,
            '.$contractor_freight.' as contractor_freight, '.$contractor_freight_amount.' as contractor_freight_amount,
            '.$contractor_freight_without_shortage.' as contractor_freight_without_shortage,
            '.$contractor_net_freight.' as contractor_net_freight, '.$contractor_commission.' as contractor_commission,
            '.$contractor_commission_amount.' as contractor_commission_amount, customers.name as customer,
            trips_details.freight_unit as freight_unit_cst, '.$total_freight_cst.' as total_freight_cst,
            '.$customer_freight.' as customer_freight, '.$customer_freight_amount.' as customer_freight_amount,
            '.$customer_freight_without_shortage.' as customer_freight_without_shortage,
            trips.id as service_charges, trips_details.bill_id, bills.billed_date_time,
             ( CASE
                    WHEN trips_details.bill_id = "0" THEN "Not Billed"
                    ELSE "Billed"
                END
             ) AS billed
        ...
        $this->db->join('trips_details_for_manage_accounts as trips_details','trips_details.trip_id = trips.id', 'left');
        ...
        $this->db->join('products','products.id = trips_details.product','left');
        $this->db->join('voucher_journal as shortage_voucher','shortage_voucher.id = trips_details.shortage_voucher','left');
