        ...
        if(isset($_POST['multiple_sort']))
        {
            ...
            for($i = 1; $i<$num_of_columns; $i++)
            {
                ...
                if(isset($_POST[$column]))
                {
                    ...
                    $column_data.="_&&_";
                    $column_data.=$_POST['type_'.$i];
                    $column_data.="_&&_";
                    $column_data.=$_POST['priority_'.$i];
                    $column_data.="_&&_";
                    $column_data.=$_POST['order_'.$i];
                    array_push($sorting_info, $column_data);
                }
            }
            $storable_string = join(',',$sorting_info);
            $data = array(
                'module'=>$sorting_module,
                'detail'=>$storable_string,
            );
            $num_of_records = $this->db->get_where('sorting',array('module'=>$sorting_module))->num_rows();
            if($num_of_records > 0)
            {
                $this->db->where('module',$sorting_module);
                $this->db->update('sorting',$data);
            }else{
