...
class Trips_model extends CI_Model {
    ...
    public function save_shortage_expense()
    {
        if($this->input->post('shortage_type') == '1'){
            ...
            $this->db->trans_start();
            if($this->accounts_model->save_voucher('users','1') == true){
    ...
    public function search_trips($keys, $limit, $start, $sort){
        ...
        if($keys['stn_number'] != '' ){
            $this->db->like('trips_details.stn_number', $keys['stn_number']);
        }
        ...
        if($keys['trip_master_type'] != '' ){
            if($keys['trip_master_type'] == 'primary'){
                $where = "(trips.type = 2 OR trips.type = 4 OR trips.type = 1 OR trips.type = 5)";
                $this->db->where($where);
        ...
        $this->db->select('trips_details.id as trips_details_ids');
        //$this->db->limit($limit, $start);
        ...
        $trips = $this->db->get()->result();
        $trips_details_ids = array();
        foreach($trips as $trip){
            array_push($trips_details_ids, $trip->trips_details_ids);
        }

        $final_trips = $this->trips_model->parametrized_trips_engine_by_detail_ids($trips_details_ids, "trips_welcome");
        usort($final_trips, array("Sorting_Model", "sort_trips"));
        $start = ($start == '')?0:$start;
    ...
    public function count_searched_trips($keys)
    {
        ...
        //$this->db->limit($limit, $start);
        $this->db->distinct('trips_details.id');
