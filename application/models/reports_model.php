...
class Reports_model extends Parent_Model {
    ...
    public function calculation_sheet_black_oil($keys)
    {
        ...
        $this->db->where(array(
            'trips.entryDate >='=>$keys['from_date'],
            'trips.entryDate <='=>$keys['to_date'],
        ));
        ...
        $details_ids_with_decanding_shortage = array();
        ...
        $result = $this->db->get('trips')->result();
        $details_ids_with_destination_shortage = array();
        foreach($result as $record)
        {
        ...
        $this->black_oil();
        if(sizeof($details_ids_with_destination_shortage) > 0)
    ...
    public function calculation_sheet_white_oil($keys)
    {
        ...
        //var_dump($details_ids_with_decanding_shortage);
        //selecting trips_details_ids which have destination shortage voucher
        $this->db->select("trips_details.id as trip_detail_id");
        $this->join_trips_and_details();
        $this->db->where('trips_details.shortage_voucher_dest !=',0);
